...
Distance between each data point to each centroid is calculated and each data point gets assigned to its nearest centroid. Once all data points have been assigned, the centroid is moved to the mean position of all the data points within its group. This is shown in the top right graph. Distances from centroids are again calculated and data points reassigned as depicted in the bottom right graph. The centroids move. See bottom left graph. This process repeats until no points change groups. 
